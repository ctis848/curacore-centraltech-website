...
export async function POST(request: Request) {
  ...
  if (event.event === 'charge.success') {
    ...
    if (amount !== expectedAmount) {
      console.error('Amount mismatch', { amount, expectedAmount, reference });
    ...
    console.log(`✅ Payment verified! Account ready for ${email} - ${plan} (${quantity} seats)`);
  }
...
export const config = {
  api: {
    bodyParser: false, // Required for webhook raw body
  },
};