...
export default function LicenseHistoryPage() {
  ...
  const [sortBy, setSortBy] = useState<"product" | "action" | "date">("date");
      ...
      <div className="overflow-x-auto border rounded-lg shadow">
        <table className="min-w-full bg-white">
          <thead className="bg-gray-100 border-b">
            <tr>
              {visibleCols.productName && <th className="px-4 py-2 text-left">Product</th>}
          ...
          <tbody>
            {paged.map((h) => (
              <tr key={h.id} className="border-b hover:bg-gray-50">
                {visibleCols.productName && <td className="px-4 py-2">{h.productName}</td>}
