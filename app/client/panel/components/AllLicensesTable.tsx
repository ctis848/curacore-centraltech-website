...
export default function AllLicensesTable() {
  ...
  useEffect(() => {
    async function load() {
      try {
        const res = await fetch("/api/client/licenses/all", {
          credentials: "include",
        });
  ...
  if (licenses.length === 0)
    return (
      <div className="p-6 bg-white dark:bg-gray-800 rounded-xl shadow border dark:border-gray-700 mt-10">
        <h3 className="text-xl font-bold mb-4">All Licenses</h3>
        <p className="text-gray-600 dark:text-gray-300">No licenses found.</p>
      </div>
    );
